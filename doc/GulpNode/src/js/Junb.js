...
var Junb=function(){
    var self=this;
    var cont=self.cont=document.getElementById("j-body");
    ...
    self.dStr='';
    self.domCtl=__domRevManifest||null;
    ...
    self.loadPage=function(id,fn){
        Junb.ajax({
            ...
            url:'dom/'+(self.domCtl?self.domCtl[id]:id)+'.html',
            type:"get",
            dataType:"string",
    ...
    self.compilePage=function(t){
        var rel=t[0].querySelectorAll("[data-href]");
        for(var i=0,len=rel.length;i<len;i++){
            !function(i){
                var v=rel[i];
                var href=v.getAttribute("data-href");
