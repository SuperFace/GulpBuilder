...
$(function(){
    ...
    !function(){
        ...
        !function(){
            $(document).on("click","[data-href]",function(){
                var str=$(this).attr("data-href").trim();
                j_m.changeHash(str);
            });
        }();
        //帖子列表点击进详情
        $(document).on("click",".comm-posts>ul>li>.comm-posts-sub",function(){
            var $itm=$(this).parent();
            j_m.changeHash("pdt&hash_id="+$itm.attr("data-id"));
        });
        //帖子列表点击进详情
        $(document).on("click",".comm-fr>ul>li>.comm-fr-sub>.comm-fr-sub-con",function(){
            var $itm=$(this).parent().parent();
            j_m.changeHash("pdt&hash_id="+$itm.attr("data-id"));
        });
        //用户头像和昵称点击进入用户主页
        $(document).on("click","[data-userid]",function(){
            j_m.changeHash("fr&hash_userid="+$(this).attr("data-userid"));
        });
        clearAllCookies();
        landscapeTip(true);//横屏提示
        wxSdkApply();//微信sdk
        responsiveScaling();
    }();
    //access
    !function(){
        ...
        j_m.extend("un_car",function(cc){
            ...
            //选择车型
            j_m.listen("hash_cate",function(data){
                if(data==="selCar"){
            ...
            $btCar.on("click",function(){
                j_m.changeHash("un_car&hash_cate=selCar");
            });
            cc();//页面渲染
        });
        ...
        j_m.extend("car",function(cc){
            ...
            j_m.listen("hash_cate",function(data){
                if(data==="selCar"){
                    chooseCar.call($btCar[0],function(data){
                        $titCar.html(data.brandname+" "+data.typename);
                        j_m.changeHash("car");
                    });
                }else{
                    chooseCar.call($btCar[0],"close");
            ...
            $btCar.on("click",function(){
                j_m.changeHash("car&hash_cate=selCar");
            });
            cc();//页面渲染
        });
        j_m.extend("task",function(cc){
            var $page=$(j_m.currPage[0]);
            cc();//页面渲染
        });
        j_m.extend("user",function(cc){
            ...
            j_m.listen("hash_cate",function(data){
                if(data==="selCar"){
                    chooseCar.call($btCar[0],function(data){
                        $btCar.html("<input type='text' class='user-info-box-t-inp inp t-right icon icon-6' value='"+data.brandname+" "+data.typename+"'/>");
                        j_m.changeHash("user");
                    });
                }else{
                    chooseCar.call($btCar[0],"close");
            ...
            $btCar.on("click",function(){
                j_m.changeHash("user&hash_cate=selCar");
            });
            cc();//页面渲染
        });
        j_m.extend("center",function(cc){
            var $page=$(j_m.currPage[0]);
            //回复楼主
            j_m.listen("hash_cate",function(data){
                if(data==="advice"){
        ...
        j_m.extend("fans",function(cc){
            var $page=$(j_m.currPage[0]);
            cc();//页面渲染
        });
        j_m.extend("sysmsg",function(cc){
            var $page=$(j_m.currPage[0]);
            cc();//页面渲染
        });
        j_m.extend("actmsg",function(cc){
            var $page=$(j_m.currPage[0]);
            cc();//页面渲染
        });
        j_m.extend("fdt",function(cc){
            ...
            $starCondP.each(function(){
                if($(this).index()===0){
                    ...
                    $(this).on("click",function(){
                        j_m.changeHash("fdt&hash_cate=reply");//回复帖子
                    });
                }
            });
            //评论点赞
            $starCondR.each(function(){
                if($(this).index()===0){
                    ...
                    $(this).on("click",function(){
                        j_m.changeHash("fdt&hash_cate=reply_rep");//回复评论
                    });
                }
            });
            //回复楼主
            ...
            $inpRep.on("touchend",function(){
                j_m.changeHash("fdt&hash_cate=reply");
            });
            cc();//页面渲染
        });
        j_m.extend("ffo",function(cc){
            var $page=$(j_m.currPage[0]);
            cc();//页面渲染
        });
        j_m.extend("ffa",function(cc){
            var $page=$(j_m.currPage[0]);
            cc();//页面渲染
        });
        j_m.extend("unit",function(cc){
            var $page=$(j_m.currPage[0]);
            cc();//页面渲染
        });
        j_m.iniPages();
...
j_m.extend("pubPre",function(t){
    ...
    Util.maiDian({key:"cwz-pv"});//页面访问量
    Util.maiDian({key:"cwz-uv"});//用户访问量
});
//公有渲染——渲染后
j_m.extend("pub",function(t){
    var page=t[0];//当前页面
    var $page=$(page);//当前页面
    //等宽高
    !function(){
...
function loadCarList(fn){
    callService({
        url:j_m.oVersion.addr.interface+"/index.php?d=pinche&c=car_sharing&m=ajax_get_car",
        type:"get",
        paras:{
        },
        callback:function(data){
